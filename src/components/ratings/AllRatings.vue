<template>
	<div class="all-ratings-wrapper">
		<div class="all-ratings-list">
			<div class="all-ratings-item"  v-for="comments in sellerRatings.comments">
				<div class="user-pic" :style="{'background-image':'url('+comments.user_pic_url+')'}"></div>
				<div class="comments-content">
					<p class="user">
						<span class="user-name">{{comments.user_name}}</span>
						<span class="comment-time">{{changeTime(comments.user_id)}}</span>
					</p>
					...
					<div class="comment_pics" v-if="comments.comment_pics.length" v-for="comment_pic in comments.comment_pics">
						<img :src="comment_pic.url">
					</div>
				</div>
			</div>
		</div>
